...
eu = conj(ue);
for initial_index = 1:floor(N/10)
% --- Optimization Setup ---
...
for i_gamma=1:14
   ...
   for i=1:Imax
       ...
       %vv=sqrt(TotalScenarios*var(weights));
       %vv=0.4*sum(exp(-(SR_vec)))/TotalScenarios;
       %mu=1-min(vv,0.9);
       vv2=exp(-min_sr_val/mu/0.5);
       if vv2>1
           vv1=1/vv2;
           vv2=1;
       else
       ...
       s_0 = exp(1i * mod(angle(vv1*s_0+vv2*LSE), 2*pi));
      
       converge((i_gamma-1)*Imax+i,initial_index)=max(min_sr_val,0);
       converge_org((i_gamma-1)*Imax+i,initial_index)=-mu*log(w_denom)+ min_sr_val;
