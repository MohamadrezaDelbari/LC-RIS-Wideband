...
for fi=56e9:1e9:56e9
    ...
    Param=struct('p_bs',p_bs,'p_irs',p_irs,'p_mu',p_mu,...
        ...
        [SNR_opt(:,:),SR_opt(t)]=SNR_calculation(Param,WW_opt{1}.^factor,0);  % ONly f1

        [SNR_opt_f_new(:,:),SR_opt_f_new(t)]=SNR_calculation(Param,WW_opt_f{1}.^factor,0);

        [SNR_opt_f_point(:,:),SR_opt_f_point(t)]=SNR_calculation(Param,WW_opt_f_point{1}.^factor,0);

        [SNR_opt_f_new(:,:),SR_opt_f_new_design(t)]=SNR_calculation(Param,WW_opt_f_new_design{1}.^factor,1);
         
        [SNR_f_scalable(:,:),SR_f_scalable(t)]=SNR_calculation(Param,WW_f_scalable{1}.^factor,1);
        t=t+1;
end
figure
fi=50e9:1e9:70e9;
...
%    'LineWidth', 5)
%title('Benchmark 2','FontSize',28,'FontWeight','bold')
...
exportgraphics(gca,fullfile('Figure','Benchmark 2.pdf'))
